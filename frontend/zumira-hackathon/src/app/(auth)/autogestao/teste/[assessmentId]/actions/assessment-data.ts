...
export async function getAssessmentData(assessmentId: string): Promise<AssessmentResponse> {
  ...
  if (error || !response.ok) {
    return { status: "ERROR", message: "Ocorreu um erro ao tentar buscar as informações deste teste. Por favor tente mais tarde" }
  }

  const parsed = (await response.json()) as GetAssessmentData

  if(parsed.data.lastCompleted !== null && isAfter(new Date(parsed.data.lastCompleted), subYears(new Date, 1))) {
