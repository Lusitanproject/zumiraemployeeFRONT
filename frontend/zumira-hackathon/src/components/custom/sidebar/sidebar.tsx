...
export function Sidebar({ menuItems, data }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "hidden md:flex flex-col justify-between h-screen bg-background-200 py-12 transition-all transition-discrete overflow-y-scroll overflow-x-visible scrollbar-hide px-12 w-[18rem] gap-4"
      )}
    >
      ...
      {!isAdminRoute && (
        <span className="flex flex-row items-center justify-center gap-1.5 text-text-500 font-semibold text-sm">
